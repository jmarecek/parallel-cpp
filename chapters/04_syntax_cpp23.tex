...
C++11 had a very basic support for threads, which required that one calls \cpp{join} or \cpp{detach} prior to the destructor being called. If neither was called, the program was \cpp{std::abort}ed. At the same time, it was almost impossible to handle exceptions while being able to call \cpp{join} correctly. The use of the C++11 \cpp{thread} is thus considered harmful and we will present only two short examples.
...
C++20 adds a new class \cpp{jthread} (``joining threads''), which does not require a call to \cpp{join} or \cpp{detach}. Instead, the destructor waits for completion of the code (``joins'') automatically. This is an example of the 
